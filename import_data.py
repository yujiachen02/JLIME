...
def import_data(data, norm_mode = 'standard'):

    df_                = data

    bin_list           = ['loan_purpose', 'cnt_borr']
    cont_list          = ['dti', 'fico', 'cltv', 'orig_upb', 'dif_rt']
    ...
    label           = pat_info[:,[2]]  #one event
    time            = pat_info[:,[1]]  #age when event occurred

    num_Category    = 60 #or specifically define larger than the max tte: int(np.max(pat_info[:, 1]) * 1.2)
...
def f_construct_nh(df, feat_list):
    '''
        id   : patient indicator
        tte  : time-to-event or time-to-censoring
            - must be synchronized based on the reference time
    ...
    max_meas = np.max(grouped.count())[0]
    data     = np.zeros([len(id_list), max_meas, len(feat_list)+1])
    pat_info = np.zeros([len(id_list)])

    for i, tmp_id in enumerate(id_list):
        tmp = grouped.get_group(tmp_id).reset_index(drop=True)

        pat_info[i] = tmp.shape[0]             #number of measurement
        data[i, :int(pat_info[i]), 1:]  = tmp[feat_list]
        data[i, :int(pat_info[i]-1), 0] = np.diff(tmp['times'])
    
    return data

def import_data_nh(data, norm_mode = 'standard'):
    ...
    cont_list          = ['dti', 'fico', 'cltv', 'orig_upb', 'dif_rt']
    feat_list          = cont_list + bin_list
    df_                = df_[['id', 'times']+feat_list]

    df_[cont_list]     = f_get_Normalization(np.asarray(df_[cont_list]).astype(float), norm_mode)

    data     = f_construct_nh(df_, feat_list)
    ...
    return DATA, data_mi
def import_dataset_nh(data, norm_mode = 'standard'):

    df_                = data

    bin_list           = ['drug', 'sex', 'ascites', 'hepatomegaly', 'spiders']
    cont_list          = ['age', 'edema', 'serBilir', 'histologic']
    feat_list          = cont_list + bin_list
    df_                = df_[['id', 'times']+feat_list]
...
def import_dataset(data, norm_mode = 'standard'):
    ...
    num_Category    = int(np.max(pat_info[:, 1]) * 1.2) #or specifically define larger than the max tte: int(np.max(pat_info[:, 1]) * 1.2)
    num_Event       = len(np.unique(label)) - 1
    max_meas = np.max(pat_info[:, 1])
    ...
    return DIM, DATA, MASK, data_mi, max_meas
